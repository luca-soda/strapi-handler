...
const testData = async () => {
    ...
    const id = await strapiHandler.findOne('nfts').filter('metadata', FilterOperator.EQUAL_TO, 'ToBeChangedToProva').chain().show('id');
    console.log(id);
    console.log(await strapiHandler.findOne('nfts').filter('id',FilterOperator.EQUAL_TO, id).chain().put({
        metadata: 'Prova'
    }));
