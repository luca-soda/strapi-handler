...
test('StrapiFindOne.or', () => __awaiter(void 0, void 0, void 0, function* () {
    yield deleteAll();
    const item = yield createRandomItem();
    const result = yield strapi.findOne(tests).filter('Str', IS_EQUAL_TO, uuidv4()).or('Str2', IS_EQUAL_TO, item.Str2).show();
    expect(result).not.toBeNull();
    expect(result.id).toBe(item.id);
}));
...
//Mega test
test('Filter tested for everything', () => __awaiter(void 0, void 0, void 0, function* () {
    yield deleteAll();
    const items = [
        {
            Str: uuidv4(),
    ...
    let result;
    // Basic filter
    let results = yield strapi.findAll(tests).filter('Num', IS_EQUAL_TO, 0).show();
    expect(results.data.length).toBe(2);
    expect(results.data[0].Num).toBe(0);
    expect(results.data[1].Num).toBe(0);
    // AND filter
    results = yield strapi.findAll(tests).filter('Num', IS_EQUAL_TO, 0).and('Str', IS_EQUAL_TO, items[0].Str).show();
    expect(results.data.length).toBe(1);
    expect(results.data[0].Num).toBe(0);
    expect(results.data[0].Str).toBe(items[0].Str);
    // OR filter
    results = yield strapi.findAll(tests).filter('Num', IS_EQUAL_TO, 0).or('Str', IS_EQUAL_TO, uuidv4()).show();
    expect(results.data.length).toBe(2);
    expect(results.data[0].Num).toBe(0);
    expect(results.data[1].Num).toBe(0);
    // All Filter Operator
    const checkWorkingOperator = ({ expectedResults, elementToCheck, valueToCheck, filterOperator, value, value2 }) => __awaiter(void 0, void 0, void 0, function* () {
        results = yield strapi.findAll(tests).filter(elementToCheck, filterOperator, value, value2).show();
        result = yield strapi.findOne(tests).filter(elementToCheck, filterOperator, value, value2).show();
